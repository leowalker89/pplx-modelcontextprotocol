...
const VALID_MODELS = ['sonar-deep-research', 'sonar-reasoning-pro', 'sonar-reasoning', 'sonar-pro', 'sonar'];
...
const MODEL_SELECTION_MAP: Record<string, ModelSelectionCriteria> = {
  'sonar-deep-research': {
    keywords: ['deep research', 'comprehensive', 'thorough', 'detailed analysis', 'expert', 'in-depth'],
    description: 'specialized for extensive research and expert-level analysis across domains'
  },
  'sonar-reasoning-pro': {
    keywords: ['reasoning', 'logic', 'solve', 'mathematical', 'technical', 'complex problem', 'figure out'],
    description: 'optimized for advanced logical reasoning and complex problem-solving'
  },
  'sonar-reasoning': {
  ...
  'sonar-pro': {
    keywords: ['search', 'find', 'lookup', 'information', 'facts', 'details', 'latest'],
    description: 'general-purpose model with excellent search capabilities and citation density'
  },
  'sonar': {
...
if (!VALID_MODELS.includes(DEFAULT_MODEL)) {
  throw new Error(`Invalid default model '${DEFAULT_MODEL}'. Valid models are: ${VALID_MODELS.join(', ')}`);
}
 ...
 */
function selectModelBasedOnIntent(query: string): { model: string; description: string } {
  // Convert query to lowercase for case-insensitive matching
  const lowercaseQuery = query.toLowerCase();
  
  // Calculate scores for each model based on keyword matches
  const modelScores = Object.entries(MODEL_SELECTION_MAP).map(([model, criteria]) => {
...
const isValidDomainArgs = (args: any): args is { domain: string; action: 'allow' | 'block' } =>
  typeof args === 'object' &&
  args !== null &&
  typeof args.domain === 'string' &&
  args.domain.trim().length > 0 &&
  ['allow', 'block'].includes(args.action);

const isValidRecencyArgs = (args: any): args is { filter: string } =>
  typeof args === 'object' &&
  args !== null &&
  typeof args.filter === 'string' &&
  ['hour', 'day', 'week', 'month', 'none'].includes(args.filter);

const isValidModelArgs = (args: any): args is { model?: string } =>
  typeof args === 'object' &&
  args !== null &&
  (args.model === undefined || 
    (typeof args.model === 'string' && VALID_MODELS.includes(args.model)));
...
class PerplexityServer {
  ...
  private async handleSearch(request: any) {
    ...
    try {
      ...
      this.domainFilters.allowedDomains.slice(0, 3).forEach(domain => {
      ...
      this.domainFilters.blockedDomains.slice(0, remainingSlots).forEach(domain => {
        domainFilterArray.push(`-${domain}`);
      });
      ...
      // Check if we're in fixed model mode or if we should auto-select
      let description: string;
      if (this.currentModel !== DEFAULT_MODEL) {
      ...
      const response = await this.axiosInstance.post<SearchResponse>('/chat/completions', apiParams);
  ...
  private handleDomainFilter(request: any) {
    ...
    const { domain, action } = request.params.arguments;
    ...
    if (action === 'allow') {
      ...
      this.domainFilters.blockedDomains = this.domainFilters.blockedDomains.filter(d => d !== cleanDomain);
      ...
      this.domainFilters.allowedDomains = this.domainFilters.allowedDomains.filter(d => d !== cleanDomain);
  ...
  private handleRecencyFilter(request: any) {
  ...
  private handleListFilters() {
    const allowedText = this.domainFilters.allowedDomains.length > 0 
      ? `Allowed domains: ${this.domainFilters.allowedDomains.join(', ')}`
      : 'No allowed domains configured.';
      
    const blockedText = this.domainFilters.blockedDomains.length > 0
    ...
    const recencyText = this.recencyFilter 
      ? `Recency filter: ${this.recencyFilter} (limiting to content from the last ${
          this.recencyFilter === 'hour' ? 'hour' : 
          this.recencyFilter === 'day' ? '24 hours' : 
          this.recencyFilter === 'week' ? '7 days' : '30 days'
        })`
      : 'No recency filter configured.';
      
    const limitText = "Note: Perplexity API supports up to 3 domains total with priority given to allowed domains.";
  ...
  private handleModelInfo(request: any) {
    ...
    // If a specific model is requested, set it
    ...
    if (model) {
      ...
      modelInfoLines.push(`Model selection is now fixed to this model until changed.`);
      modelInfoLines.push('');
    } else {
      modelInfoLines.push(`Current model: ${this.currentModel}`);
      modelInfoLines.push(`Default model (from environment): ${DEFAULT_MODEL}`);
      modelInfoLines.push(`Automatic model selection: ${this.currentModel !== DEFAULT_MODEL ? 'DISABLED' : 'ENABLED'}`);
      modelInfoLines.push('');
    }
    
    // List available models with descriptions
    modelInfoLines.push('Available models:');
    Object.entries(MODEL_SELECTION_MAP).forEach(([modelName, criteria]) => {
    ...
    // Instructions
    modelInfoLines.push('');
    modelInfoLines.push('To use automatic model selection: Simply search without setting a specific model.');
    modelInfoLines.push('To set a specific model: Use this tool with the "model" parameter.');
  ...
  async run() {
    ...
    console.error(`Perplexity Search MCP server running on stdio (default model: ${DEFAULT_MODEL})`);
